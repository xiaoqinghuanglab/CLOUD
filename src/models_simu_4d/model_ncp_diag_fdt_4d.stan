data {
    ...
    vector<lower=0>[N] deltat;        // Nonnegative time gaps for valid transition variances
...
parameters {
    ...
    // Diagonal drift rates.  For diagonal Gamma, positive diagonal
    // entries are exactly the positive-stability condition.
    vector<lower=1e-6>[R] gamma_diag;
}
...
transformed parameters {
    ...
    // Diagonal structural matrices.
    // Omega is constrained to be both diagonal and a correlation matrix;
    // therefore Omega must be the identity matrix.
    Omega = identity_matrix(R);

    // Positive diagonal Gamma is positive stable because its eigenvalues
    // are gamma_diag[1], ..., gamma_diag[R], all constrained > 0.
    Gamma = diag_matrix(gamma_diag);

    // With stationary covariance Omega = I and diagonal Gamma, the
    // diffusion matrix is Sigma = Gamma * Omega + Omega * Gamma' = 2 * Gamma.
    // The generated quantities block reports Sigma = 2 * S, so set S = Gamma.
    S = Gamma;
    ...
    // --- NON-CENTERED JOINT SAMPLER: Deterministic Construction ---
    {
        for (i in 1:Nsub) {
            ...
            // Time = 1.  Since Omega = I, its Cholesky factor is also I.
            vector[q] z0 = Z[start_idx]';
            vector[R] mu_start = (A_latent * z0 + c_latent) * time[start_idx];
            xi[:, start_idx] = mu_start + xi_raw[:, start_idx];

            // Time = 2 to end
            for (j in 2:repme[i]) {
                int k = start_idx + j - 1;
                vector[R] phi_diag = exp(-deltat[k] * gamma_diag);
                matrix[R, R] Phi = diag_matrix(phi_diag);
                vector[R] q_diag = rep_vector(1.0, R) - phi_diag .* phi_diag;
                matrix[R, R] L_Q = diag_matrix(sqrt(q_diag + rep_vector(1e-6, R)));
                ...
                // xi[k] = mean + diagonal transition SD * standard normal noise.
...
generated quantities {
    ...
    // For Omega = I and diagonal Gamma, Sigma = 2 * Gamma.
    Sigma = 2 * S;

    // Safely decompose Sigma for downstream Python evaluation scripts.
    L_Sigma = cholesky_decompose(add_diag(Sigma, 1e-6));

    // Omega is diagonal identity, so all upper-triangular correlations are zero.
    rho = rep_vector(0.0, 6);

    A05 = diag_matrix(exp(-0.5 * gamma_diag));
    Cova_trans05 = Omega - A05 * Omega * A05';

    A10 = diag_matrix(exp(-gamma_diag));
    Cova_trans10 = Omega - A10 * Omega * A10';

    A15 = diag_matrix(exp(-1.5 * gamma_diag));
    Cova_trans15 = Omega - A15 * Omega * A15';
/*
