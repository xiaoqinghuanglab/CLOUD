// Model 2a: Longitudinal IRT with SDE Latent Process & Covariates (Joint Latent Sampler - Lyapunov Refactor)
...
parameters {
    ...
    // --- CHANGED: Diagonal Elements for Gamma ---
...
transformed parameters {
    ...
    // --- CHANGED: Construct Gamma and Omega as strictly diagonal matrices ---
    Gamma = diag_matrix(gamma_diag);
    
    // A diagonal correlation matrix is exactly the Identity matrix
    Omega = diag_matrix(rep_vector(1.0, R)); 

    // Because Omega is Identity and A (skew) must be 0 for Gamma to be diagonal
    S = Gamma;
    ...
    // --- NON-CENTERED JOINT SAMPLER: Deterministic Construction ---
    {
        for (i in 1:Nsub) {
            ...
            // --- CHANGED: Since Omega is Identity, its Cholesky factor is also Identity
            ...
            for (j in 2:repme[i]) {
                ...
                // --- CHANGED: Optimized Matrix Exponential for Diagonal Matrices ---
                matrix[R, R] Phi = diag_matrix(exp(-deltat[k] * gamma_diag));
...
generated quantities {
    ...
    // (Note: Since Omega is strictly diagonal, this evaluates to 0.0)
    rho = Omega[1, 2];

    // --- CHANGED: Optimized for diagonal matrices ---
    matrix[R, R] A05 = diag_matrix(exp(-0.5 * gamma_diag));
    matrix[R, R] Cova_trans05 = Omega - A05 * Omega * A05';

    matrix[R, R] A10 = diag_matrix(exp(-1.0 * gamma_diag));
    matrix[R, R] Cova_trans10 = Omega - A10 * Omega * A10';

    matrix[R, R] A15 = diag_matrix(exp(-1.5 * gamma_diag));
